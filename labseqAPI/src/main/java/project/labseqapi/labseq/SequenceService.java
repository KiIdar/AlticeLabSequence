...
public class SequenceService {
    ...
    public ResponseDTO calculateLabSeq(int n) {
        ...
        if (closestStart != 0) {
            long endTime = System.nanoTime();
            return new ResponseDTO(endTime - startTime, calculateFromCache(n, closestStart));
        } else {
            long endTime = System.nanoTime();
            return new ResponseDTO(endTime - startTime, calculateFromScratch(n));
        }
    }

    // Check if there is a cached value that we can start of from saving us computation power
